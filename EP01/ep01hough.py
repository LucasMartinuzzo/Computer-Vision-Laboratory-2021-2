...
def markersDetector(image):
    ...
    max_radius = 15
    img_blur = cv2.medianBlur(image, 5)
    circles_all = cv2.HoughCircles(img_blur, cv2.HOUGH_GRADIENT,
                                   1,100,
                                   param1=100,param2=10,
                                   minRadius=min_radius,
                                   maxRadius=max_radius)
    circles_all = np.int16(np.around(circles_all))
    circles = [circle for circle in circles_all[0, :]]
    height, width = image.shape
    # Filtering only the markers
    top_left_circle = [width/2,height/2,10]
    top_right_circle = [width/2,height/2,10]
    bottom_left_circle = [width/2,height/2,10]
    bottom_right_circle = [width/2,height/2,10]
    filtered_marker_circles = []
    for circle in circles:
        if circle[0] < top_left_circle[0] and circle[1] < top_left_circle[1]:
                top_left_circle = circle
        if circle[0] < bottom_left_circle[0] and circle[1] > bottom_left_circle[1]:
                bottom_left_circle = circle
        if circle[0] > top_right_circle[0] and circle[1] < top_right_circle[1]:
                top_right_circle = circle
        if circle[0] > bottom_right_circle[0] and circle[1] > bottom_right_circle[1]:
                bottom_right_circle = circle
    filtered_marker_circles = [top_left_circle,top_right_circle,
                               bottom_left_circle,bottom_right_circle]
    return filtered_marker_circles
...
def readCode(image,marker_keypoints):
    ...
    W = top_right_keypoint[0] - top_left_keypoint[0]
    top_left_code = np.array([top_left_keypoint[0] + W*0.259, top_left_keypoint[1] + W*(-0.0435)])
    bot_left_code = np.array([top_left_keypoint[0] + W*0.259, top_left_keypoint[1] + W*(-0.0208)])
...
def readNUSP(marker_keypoints,nusp_keypoints):
    ...
    top_right_keypoint = marker_keypoints[1]
    W = top_right_keypoint[0] - top_left_keypoint[0]
    top_left_nusp = np.array([top_left_keypoint[0] + W*0.0456, top_left_keypoint[1] + W*(0.111)])
    bottom_right_nusp = np.array([top_left_keypoint[0] + W*0.2484, top_left_keypoint[1] + W*(0.4049)])
...
def main():
    ...
    #Pequena operação morphologica para deixar o preto mais forte
    ...
    diff_y = np.abs(top_left_keypoint[1] - bottom_left_keypoint[1])
    diff_y = np.abs(diff_y)
    diff_x = top_left_keypoint[0] - bottom_left_keypoint[0]
    #print(diff_x, diff_y)
    ...
    for keypoint in code_keypoints:
        if keypoint.pt[1] < top_left_keypoint[1]:
